...
const APPLICATION_SERVER_URL = process.env.NODE_ENV === 'production' ? '' : 'http://103.90.228.54:5443/openvidu/';
...
class VideoRoomComponent extends Component {
    constructor(props) {
        ...
        let sessionName = this.props.sessionName ? this.props.sessionName : Math.floor(Math.random() * 9999).toString();
    ...
    connect(token) {
        console.log(this.state.session.connect);
        this.state.session
            .connect(
                token.token,
    ...
    async getToken() {
        ...
        return await this.createToken(sessionId.id);
    }

    async createSession(sessionId) {
        const code = btoa("OPENVIDUAPP:" + "44213f14-4869-4fb8-8ff0-9a9720dbcc06");
        const response = await axios.post(APPLICATION_SERVER_URL + 'api/sessions', { customSessionId: sessionId }, {
            headers: { 
                'Content-Type': 'application/json', 
                "Authorization": "Basic " + btoa("OPENVIDUAPP:" + "44213f14-4869-4fb8-8ff0-9a9720dbcc06"),
